...
- (void)handleClient:(int)clientSocket {
    ...
    if ([request hasPrefix:@"GET / HTTP"]) {
        ...
        response = @"<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>手电筒控制</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,sans-serif;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}.container{background:rgba(255,255,255,0.95);border-radius:24px;padding:40px;max-width:400px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3)}h1{font-size:28px;margin-bottom:30px;text-align:center;color:#333}.status{text-align:center;margin-bottom:30px;font-size:18px;color:#666}.toggle-btn{width:100%;padding:20px;font-size:20px;font-weight:600;border:none;border-radius:16px;cursor:pointer;transition:all 0.3s;margin-bottom:20px}.btn-on{background:#4CAF50;color:white}.btn-on:active{background:#45a049}.btn-off{background:#f44336;color:white}.btn-off:active{background:#da190b}.slider-container{margin-top:20px}input[type=range]{width:100%;height:8px;border-radius:4px;background:#ddd;outline:none;-webkit-appearance:none}input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:24px;height:24px;border-radius:50%;background:#667eea;cursor:pointer}.level-display{text-align:center;margin-top:10px;font-size:16px;color:#666}</style></head><body><div class='container'><h1>🔦 手电筒控制</h1><div class='status' id='status'>加载中...</div><button id='toggle' class='toggle-btn' onclick='doToggle()'>切换</button><div class='slider-container'><input type='range' id='slider' min='1' max='100' value='100' oninput='setLevel(this.value)'><div class='level-display'>亮度: <span id='level'>100</span>%</div></div></div><script>function updateStatus(){fetch('/status').then(r=>r.json()).then(d=>{document.getElementById('status').textContent=d.isOn?'状态: 已开启':'状态: 已关闭';document.getElementById('toggle').textContent=d.isOn?'关闭':'开启';document.getElementById('toggle').className='toggle-btn '+(d.isOn?'btn-off':'btn-on');document.getElementById('level').textContent=Math.round(d.level*100)}).catch(e=>console.error(e))}function doToggle(){fetch('/toggle',{method:'POST'}).then(()=>updateStatus())}function setLevel(v){fetch('/level',{method:'POST',body:v/100}).then(()=>updateStatus())}updateStatus();setInterval(updateStatus,2000)</script></body></html>";
